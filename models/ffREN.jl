...
# Flux.trainable(L::implicit_ff_cell) = [L.ρ, L.V, L.S_1, L.B2, L.D12, L.bx, L.bv]
Flux.trainable(L::implicit_ff_cell) = filter(p -> length(p) !=0, [L.ρ, L.V, L.S_1, L.B2, L.D12, L.bx, L.bv])
...
function implicit_ff_cell{T}(nu, nx, nv; nl=relu, ϵ=0.01, bx_scale=0.0, bv_scale=1.0, polar_param=false) where T
    ...
    Λ = 2*I  #Choose sum abs values to make diagonally dominant
    H22 = 2Λ - D11 - D11'
    Htild = [(E + E' - P) -C1' F';
             -C1 H22 B1'
             F B1  P] + ϵ * I
    ...
    return implicit_ff_cell{T}(nl, ρ, V, S_1, B2, D12, bx, bv, ϵ, polar_param)
end
...
function sample_ff_ren(nu, nx, nv; nl=relu, ϵ=0.01, bx_scale=0.0, bv_scale=1.0, polar_param=false)
    ...
    # Make orthogonal V
    V = randn(2nx + nv, 2nx + nv) / sqrt(2nx + nv)
    V = Matrix(qr(V).Q)
    ...
    return implicit_ff_cell(nl, ρ ,V, S_1, B2, D12, bx, bv, ϵ, polar_param)
end
...
function explicit(model::implicit_ff_cell)
    ...
    ρ = model.ρ
    V = model.V
    H = model.polar_param ? exp(ρ[1])*V'*V / norm(V)^2 : V'*V + ϵ * I
...
# Filter out zero sizes so this works nicely when nx=0
Flux.trainable(model::dissipative_ff_rnn) = filter(
    p -> length(p) !=0, 
    (Flux.trainable(model.implicit_cell)..., Flux.trainable(model.output)...)
)
...
function bounded_ff_rnn(T::DataType, nu, nx, nv, ny, γ; 
    ...
    return dissipative_ff_rnn{T}(nu, nx, nv, ny, Q, S, R; nl=nl, ϵ=0.001, bx_scale=0.0, bv_scale=1.0) 
end
...
function passive_ff_rnn(T::DataType, nu, nx, nv, ny; 
    ...
    S = Matrix(I / 2, nu, ny)
    Q = Matrix(-η * I, nu, nu)
    return dissipative_ff_rnn(nu, nx, nv, ny, Q, S, R; nl=nl, ϵ=0.001, bx_scale=0.0, bv_scale=1.0) 
end

# set the output_layer to zero
function set_output_zero!(model::dissipative_ff_rnn)
    model.output.C2  = 0*model.output.C2
    model.output.D21 = 0*model.output.D21
    model.output.D22 = 0*model.output.D22
    model.output.by  = 0*model.output.by
end
...
"""
    This is some sample code demonstrating the stable REN for the youla parameterization
"""
# Dissipative feedforward RNN code
mutable struct io_ren{T}
    nu
    nx
    nv
    ny
...
# Filter out zero sizes so this works nicely when nx=0
Flux.trainable(model::io_ren) = filter(
    p -> length(p) !=0, 
    (Flux.trainable(model.implicit_cell)..., Flux.trainable(model.output)..., model.output.D22)
)

# Constructors
function io_ren{T}(nu, nx, nv, ny; nl=relu, ϵ=0.001, bx_scale=0.0, bv_scale=1.0, polar_param=true) where T
    implicit_params = sample_ff_ren(nu, nx, nv; nl=nl, 
                                    ϵ=ϵ, bx_scale=bx_scale,
                                    bv_scale=bv_scale, polar_param=polar_param)

    output_params = output{T}(nu, nx, nv, ny)
    return io_ren{T}(nu, nx, nv, ny, implicit_params, output_params)
end

function init_state(model::io_ren{T}, batches) where T
    return typeof(model.implicit_cell.V)(zeros(T, model.nx, batches))
end

function (model::io_ren)(x0, ut)
    exp_cell = explicit(model.implicit_cell)
    xt, wt = simulate(exp_cell, x0, ut)
    return model.output.(xt, wt, ut), xt[end]
end

function set_output_zero!(model::io_ren)
    model.output.C2  .*= 0
    model.output.D21 .*= 0
    model.output.D22 .*= 0
    model.output.by  .*= 0
...
function io_ren_test()
    model = io_ren{Float64}(1, 5, 50, 1; nl=Flux.relu, ϵ=1E-1, bx_scale=0.0, bv_scale=1.0, polar_param=true)

    # Check number of params
    n_params = length(Flux.params(model))
    @assert(n_params == 11, @sprintf("Found %d params", n_params))

    @assert(isa(explicit(model.implicit_cell), explicit_ff_cell),
                "explicit does not create and explicit cell")

    u = [randn(1, 5) for i in 1:100]
    x0 = init_state(model, 5)
    y = model(x0, u)[1]
end